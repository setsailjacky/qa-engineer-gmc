...
test("Should include a readable timestamp", () => {
  ...
  expect(dateTimeElement).toHaveAttribute(
    "dateTime",
    dayjs.unix(dateTimeMessage.createdAt).format("YYYY-MM-DD HH:mm")
  );
  expect(dateTimeElement).toContainHTML(
    dayjs.unix(dateTimeMessage.createdAt).format("YYYY MMM Do h:mm A")
  );
});
